...
CATEGORY_SOURCES = (
    ("robot-tests", "robot", "robot_test"),
    ("custom-libs", "custom_libs", "custom_lib"),
    ("report-example", "output", "report_example"),
    ("successful-execution", "success", "successful_execution"),
    ("failed-execution", "fail", "failed_execution"),
    ("lld-examples", "LLD", "lld_example"),
)
...
def sanitize_sensitive_repo(
    *,
    ...
    for folder_name, source_name, file_prefix in CATEGORY_SOURCES:
        ...
        category_summaries[folder_name] = {
            "files": len(entries),
            "redactions": sum(
                int(entry["redaction_total"]) for entry in entries
            ),
        }
        _write_category_readme(target_dir, folder_name, len(entries))

    manifest = {
        "schema_version": "aegis-sanitized-data.v1",
        "generated_at": datetime.now(UTC).isoformat(),
        "source": "quarantined_sensitive_reference",
        "target_layout": [folder_name for folder_name, _, _ in CATEGORY_SOURCES],
        "safety": {
