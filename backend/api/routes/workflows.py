...
class IntelligenceConfigRequest(BaseModel):
...
def run_and_persist_workflow_start(
    *,
    ...
    intelligence: IntelligenceConfigRequest | IntelligenceConfigBlock | None = None,
) -> TestContext:
    intelligence_config = build_intelligence_config(intelligence)
    ...
    metadata = {
        ...
        "llm_provider": completed_context.intelligence_config.llm_provider,
...
def build_intelligence_config(
    request: IntelligenceConfigRequest | IntelligenceConfigBlock | None,
) -> IntelligenceConfigBlock:
    if isinstance(request, IntelligenceConfigBlock):
        return request

    llm_provider = request.llm_provider if request and request.llm_provider else settings.default_llm_provider
    embedding_provider = (
        request.embedding_provider
        if request and request.embedding_provider
